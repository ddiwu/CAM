...
extern "C" {
void krnl_output(ap_uint<32>* mem, 
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_1,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_2,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_3,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_4,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_5,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_6,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_7,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_8,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_9,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_10,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_11,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_12,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_13,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_14,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_15,
               hls::stream<ap_uint<DATA_WIDTH>>& block_in_16) {
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    static ap_uint<32> match_count[16] = {0};
#pragma HLS ARRAY_PARTITION variable=match_count complete dim=1
...
#pragma HLS UNROLL
    ...
    hls::stream<ap_uint<DATA_WIDTH>>* streams[16] = {
...
#pragma HLS ARRAY_PARTITION variable=done complete dim=1
    ...
    output_loop: while (true) {
    #pragma HLS pipeline II=1
        // Process each stream
        for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
            if ((!done[i]) && (!streams[i]->empty())) {
                ap_uint<DATA_WIDTH> v = streams[i]->read();
    ...
    // Sum up the match values and write to mem[0]
    ap_uint<32> total_count = 0;
    for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
        total_count += match_count[i];
    }
