...
//test
void krnl_output_SLR1(ap_uint<512>* out, int size, hls::stream<ap_uint<512>>& outStream1){
    ...
    mem_wr:
        for (int i = 0; i < size; i++)
        {
            ...
            bool valid = (result[0] != 511) ;//|| (result[1] != 511) ||
                            // (result[2] != 511); //|| (result[3] != 511) ||
                            // (result[4] != 511) || (result[5] != 511);
            if (valid) {
                out[i] = 100;
            }
        }
